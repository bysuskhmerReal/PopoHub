...
local whatsNew = {
    ...
    "Auto check Gui new for font game and Gui",
    "Edit Teleport Map Afk",
    "New Script Fly Pad",
    "Remove TAG ALL",
    "Auto Save Notification In Folders"
}
...
-- ======= Config =======
local fileName = "notifications.json"
local fullPath = folderName .. "/" .. fileName

-- Load settings
local notificationV = {
    notificationPlayerJoinGame = false,
    notificationPlayerLeaveGame = false,
    notificationfriend = false
}
...
-- Use notificationV directly as Default
local ToggleJoin = t1:Toggle({
    Title = "Notification Player Join",
    Desc = "Show when someone joins the game",
    Type = "Checkbox",
    Default = notificationV.notificationPlayerJoinGame, -- ✅ use loaded value
    Callback = function(state)
...
local ToggleLeave = t1:Toggle({
    ...
    Type = "Checkbox",
    Default = notificationV.notificationPlayerLeaveGame, -- ✅ use loaded value
    Callback = function(state)
...
local ToggleFriend = t1:Toggle({
    ...
    Type = "Checkbox",
    Default = notificationV.notificationfriend, -- ✅ use loaded value
    Callback = function(state)
                    ...
                    if game:GetService("StarterGui"):FindFirstChild("SetCore") then
                        pcall(function()
                            game.StarterGui:SetCore("SendNotification", {
                                ...
                                Text = "Prompt '"..v.Name.."' HoldDuration = 0",
                                Duration = 3
                            })
                        end)
                    end
...
workspace.DescendantAdded:Connect(function(inst)
    if ToggleDestroy and inst:IsA("ProximityPrompt") then
        inst.HoldDuration = 0
        -- បង្ហាញ notification
        if game:GetService("StarterGui"):FindFirstChild("SetCore") then
            pcall(function()
                game.StarterGui:SetCore("SendNotification", {
                    Title = "Prompt Updated",
                    Text = "Prompt '"..inst.Name.."' HoldDuration = 0",
                    Duration = 3
                })
            end)
        end
